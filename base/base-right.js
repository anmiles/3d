...
function main() {
    return cube({size: [width, (space + pillar + join[1]) / 2, height]})
        ...
        .subtract(cube({size: join})
            .translate([(width - join[0]) / 2, (space + pillar - join[1]) / 2, base + deep]))
        .union(cube({size: [width, join[1], base]})
            .translate([0, (space + pillar - join[1]) / 2, 0]))
        .subtract(cyls(screw[0], height)
            .translate([0, (space + pillar) / 2, 0]))
        .subtract(cyls(Math.max(screwhead[0], shim[0]), roof - (screw[1] - screwhead[1] - 2 * shim[1] - nut[1] - join[2]) / 2)
            .translate([0, (space + pillar) / 2, 0]))
        .subtract(cyls(Math.max(nut[0], shim[0]), height)
            .translate([0, (space + pillar) / 2, - (roof + join[2] + (screw[1] - screwhead[1] - 2 * shim[1] - nut[1] - join[2]) / 2)]));
}
